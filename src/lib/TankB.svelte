<script lang="ts">
	...
	useTask((delta) => {
		...
		angularVelocity.set(0, controls.axisB.x * -1 * angularSpeed, 0);

		// calculate the forward velocity
		linearVelocity.set(0, 0, controls.axisB.y * -1 * linearSpeed);
		...
		if (controls.axisB.x !== 0) {
			rigidBody.setAngvel(angularVelocity, true);
		}
		if (controls.axisB.y !== 0) {
			if (controls.boostB) {
